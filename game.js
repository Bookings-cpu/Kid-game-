...
const SOUV_ICONS = ['🌼', '🌵', '❄️', '🍭', '🌋', '🌴', '🌙'];
// escalating combo praise — hoisted so it isn't re-allocated on every coin
...
function startRun(dailyMode) {
  ...
  Object.assign(G, {
    ...
    fever: 0, flashT: 0, nextMile: 500, queueJump: false,
    jumpDur: 0.62, jumpPow: 1, trail: [],
    guardD: 0.4, guardLane: 0, heartT: 0, caught: false, stumbled: false,
...
function spawnChunk() {
  ...
  let cand = PATTERNS.filter(p => {
    if (easy) {
      if (p.filter(k => k === 'train').length > 1) return false;
      if (!p.includes(null)) return false; // early game always keeps one free, no-action lane
    }
...
function update(dt) {
  G.phase += dt;
  // speed ramps up forever — identical early curve, but keeps climbing past the
  // old ~13k plateau up to a higher cap so skilled runs never get boring
  const targetSpeed = Math.min(900, 300 + G.dist / 28);
  ...
  for (const o of G.obstacles) {
    ...
    if (o.z < HITZ && o.z + o.len > -12 && Math.abs(o.lane - G.laneF) < 0.45) {
      ...
      // hurdles, bars & drones make you STUMBLE — each one lets the guard surge
      ...
      if (o.kind !== 'train') {
        ...
        if (G.guardD >= 1.0) { // caught up — GOTCHA!
          G.caught = true;
          crash();
          break;
        }
...
function crash() {
  ...
  if (G.caught) {
    G.guardD = 1.35; // lunge right onto the player
    G.guardLane = G.laneF;
...
function revive() {
  ...
  G.state = 'playing';
  G.guardD = 0.45; // he's close after a revive — sprint to shake him!
  G.caught = false;
  G.stumbled = false;
  AudioSys.sfx('revive');
...
/* ---------- cached vignette (geometry only depends on W/H) ---------- */
let _vigGrad = null, _vigKey = '';
function vignette(alpha) {
  const key = W + 'x' + H + ':' + alpha;
  if (_vigKey !== key) {
    _vigGrad = ctx.createRadialGradient(W / 2, H / 2, Math.min(W, H) * 0.45, W / 2, H / 2, Math.max(W, H) * 0.78);
    _vigGrad.addColorStop(0, 'rgba(0,0,0,0)');
    _vigGrad.addColorStop(1, `rgba(0,0,0,${alpha})`);
    _vigKey = key;
  }
  return _vigGrad;
}
...
let _bloomC = null, _bloomX = null, _bloomBW = 0, _bloomBH = 0;
function bloomPass() {
  const bw = Math.max(1, Math.round(W / 4)), bh = Math.max(1, Math.round(H / 4));
  if (!_bloomC || _bloomBW !== bw || _bloomBH !== bh) {
    _bloomC = document.createElement('canvas');
    _bloomC.width = bw; _bloomC.height = bh;
    _bloomX = _bloomC.getContext('2d');
    _bloomBW = bw; _bloomBH = bh;
  }
  ...
  ctx.drawImage(_bloomC, 0, 0, bw, bh, 0, 0, W, H);
  ctx.filter = 'none';
  ctx.globalAlpha = 1;
