...
window.R3D = (() => {
  ...
  function makeCoin() {
    const geo = new THREE.CylinderGeometry(0.3, 0.3, 0.07, 18);
    geo.rotateX(Math.PI / 2);
    const m = new THREE.Mesh(geo, mat('#ffd23e', { metalness: 0.55, roughness: 0.3, emissive: '#8a6200', emissiveIntensity: 0.35 }));
    m.castShadow = true;
    return m;
  }
  ...
  function init(canvas) {
    ...
    scene.add(new THREE.HemisphereLight(0xffffff, 0x668855, 0.55));
    ...
    trackBed = new THREE.Mesh(new THREE.BoxGeometry(LANE * 3 + 1.6, 0.12, 70), mat('#9a9088'));
    ...
    for (const lane of [-1, 0, 1]) {
      for (const off of [-0.42, 0.42]) {
        const rail = new THREE.Mesh(new THREE.BoxGeometry(0.08, 0.09, 70),
          mat('#cfd6e0', { metalness: 0.7, roughness: 0.35 }));
    ...
    for (let i = 0; i < 46; i++) {
      const sl = new THREE.Mesh(new THREE.BoxGeometry(LANE * 3 + 1.2, 0.07, 0.28), mat('#6e4f33'));
      sl.position.y = 0.09;
    ...
    const mountains = new THREE.Mesh(new THREE.PlaneGeometry(220, 26),
      new THREE.MeshBasicMaterial({ map: mountainTex, transparent: true, fog: false, depthWrite: false, opacity: 0.85 }));
    mountains.position.set(0, 8, -58);
    scene.add(mountains);

    /* ----- continuous trackside scenery rows ----- */
    const AMBIENT_KINDS = ['tree', 'tree', 'pine', 'house', 'tree', 'rock', 'tree', 'house'];
    for (const side of [-1, 1]) {
      for (let i = 0; i < 16; i++) { // near row
        const kind = AMBIENT_KINDS[Math.floor(hash01(i * 17 + side * 3) * AMBIENT_KINDS.length)];
        const m = makeDecor(kind);
        const sc = 0.9 + hash01(i * 23 + side) * 0.7;
        m.scale.setScalar(sc);
        m.userData.amb = { side, i, spacing: 4.2, x: side * (5.8 + hash01(i * 31 + side) * 3.5) };
        scene.add(m);
        ambient.push(m);
      }
      for (let i = 0; i < 12; i++) { // far row, bigger
        const kind = AMBIENT_KINDS[Math.floor(hash01(i * 41 + side * 7) * AMBIENT_KINDS.length)];
        const m = makeDecor(kind);
  ...
  function render(o) {
    ...
    camera.lookAt(sway * 0.85, 0.75, -9);
