...
window.R3D = (() => {
  ...
  let ground, trackBed;
  let skyCanvas, skyCtx, skyTexture, sunSprite, cloudMat;
  ...
  const duckTex = spriteTex((x, s) => {
    x.fillStyle = '#e8443a'; x.fillRect(0, 0, s, s); // whole sign red (was half → black lower half)
    x.fillStyle = '#fff';
    x.font = `800 ${s * 0.34}px "Baloo 2","Comic Sans MS",sans-serif`;
    x.textAlign = 'center'; x.textBaseline = 'middle';
    x.fillText('DUCK!', s / 2, s / 2);
  }, 128);
  ...
  // inverted-hull outline clone of a mesh, so EVERY limb gets its own dark edge
  // (not just the body) — the silhouette then reads as one connected character.
  function outlineOf(mesh, inflate) {
  ...
  function makeChar(def) {
    const g = new THREE.Group();
    // cartoon outline: an inflated near-black backside shell, inflated *proportionally*
    // to the body so it hugs evenly instead of reading as a fat hoop
    const outline = new THREE.Mesh(new THREE.SphereGeometry(0.55, 18, 14), OUTLINE);
    outline.scale.set(1.08, 1.17, 0.92);
    ...
    switch (def.hat) {
      case 'cap': {
        const top = new THREE.Mesh(new THREE.SphereGeometry(0.4, 14, 10, 0, Math.PI * 2, 0, Math.PI / 2), mat('#e23a30'));
        top.position.y = 1.46;
        g.add(outlineOf(top, 1.1)); // dark edge so the red cap reads against an orange body
        g.add(top);
        const brim = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.06, 0.35), mat('#e23a30'));
        brim.position.set(0, 1.47, 0.4);
        g.add(outlineOf(brim, 1.18));
        g.add(brim);
        break;
      }
      case 'ears':
      ...
      case 'bolt': {
        ...
        b.position.y = 1.78; // sits above the head, not over the face
        b.rotation.z = 0.4;
        g.add(b);
        break;
      }
      ...
      case 'guardcap': { // dark peaked police cap — distinct chaser silhouette
        const top = new THREE.Mesh(new THREE.SphereGeometry(0.42, 14, 10, 0, Math.PI * 2, 0, Math.PI / 2), mat('#26304a'));
        top.position.y = 1.46; top.scale.y = 0.72;
        g.add(top);
        const brim = new THREE.Mesh(new THREE.BoxGeometry(0.58, 0.06, 0.42), mat('#171f30'));
        brim.position.set(0, 1.4, 0.42);
        g.add(brim);
        const badge = new THREE.Mesh(new THREE.CircleGeometry(0.07, 10), mat('#ffd23e', { emissive: '#caa400', emissiveIntensity: 0.6 }));
        badge.position.set(0, 1.54, 0.33);
        g.add(badge);
        break;
      }
    }

    // limbs: pivoted capsules with shoe / mitten tips — each gets its own outline
    ...
    for (const s of [-1, 1]) {
      ...
      sh.position.set(s * 0.5, 1.05, 0.06); // a touch forward so the swing is visible
      const arm = new THREE.Mesh(new THREE.CapsuleGeometry(0.105, 0.3, 4, 8), mat(def.body));
      arm.position.y = -0.21;
      sh.add(outlineOf(arm, 1.3));
      sh.add(arm);
      const hand = new THREE.Mesh(new THREE.SphereGeometry(0.13, 10, 8), mat(def.belly));
      hand.position.y = -0.42;
      sh.add(outlineOf(hand, 1.22));
      sh.add(hand);
      g.add(sh);
  ...
  function animateChar(g, o) {
    ...
    // crash: tumble over backwards with limbs flailing (o.dying ramps 0 -> 1)
    if (o.dying > 0) {
      const t = o.dying;
      g.scale.y = 1;
      g.rotation.x = -1.4 * t;                       // fall onto his back
      g.rotation.z = 0.5 * Math.sin(t * 9) * (1 - t); // wobble as he goes down
      for (let i = 0; i < 2; i++) {
        const hip = limbs.hips[i], sh = limbs.shoulders[i];
        hip.visible = sh.visible = true;
        hip.rotation.x = -1.0 * t + (i ? 1 : -1) * 0.4 * t;
        sh.rotation.x = -2.4 * t;
        sh.rotation.z = (i ? 1 : -1) * (0.25 + 1.9 * t);
      }
      body.scale.x = 1;
      return;
    }
    ...
    for (let i = 0; i < 2; i++) {
      ...
      if (o.jump > 0.3) {
        ...
        // the guard reaching forward to grab you — not a celebratory jump
        hip.rotation.x = ph * 1.05;
        sh.rotation.z = (i ? 1 : -1) * 0.22;
        sh.rotation.x = -1.5 - o.lunge * 0.7;
      } else {
  ...
  function init(canvas) {
    ...
    const deck = new THREE.Mesh(new THREE.BoxGeometry(1.15, 0.16, 0.62),
      mat('#3fd0ff', { emissive: '#1f8fe0', emissiveIntensity: 0.7 }));
    board.add(outlineOf(deck, 1.06));
    board.add(deck);
    const underglow = new THREE.Mesh(new THREE.BoxGeometry(1.0, 0.06, 0.5),
      new THREE.MeshBasicMaterial({ color: 0x7ff2ff, transparent: true, opacity: 0.6, blending: THREE.AdditiveBlending, depthWrite: false }));
    underglow.position.y = -0.14;
    board.add(underglow);
    board.userData.glow = underglow;
    board.visible = false;
    scene.add(board);
    ...
    cloudMat = new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0.92, fog: false });
    for (let i = 0; i < 7; i++) {
  ...
  function ensureChar(def) {
    ...
    if (!guard) {
      guard = makeChar({ id: 'guard', body: '#6f7891', belly: '#b9c2d2', hat: 'guardcap' });
  ...
  function render(o) {
    ...
    // the railroad bed + ties follow the biome (a darker, desaturated version of
    // the ground) so it isn't a brown track running through snow, lava or space
    _c1.setStyle(o.ground).offsetHSL(0, -0.12, -0.13);
    trackBed.material.color.copy(_c1);
    _c1.offsetHSL(0, 0, -0.05);
    for (const sl of sleepers) sl.material.color.copy(_c1);
    // fade out the daytime sun + clouds as we approach Moon Base (night/space)
    const day = 1 - (o.night || 0);
    if (sunSprite) sunSprite.material.opacity = day;
    if (cloudMat) cloudMat.opacity = 0.92 * day;
    ...
    animateChar(charMesh, {
      ...
      dying: G.state === 'dying' ? (1 - Math.max(0, G.dieT) / 0.9) : 0,
    });
    ...
    if (shieldBall.visible) {
      ...
      shieldBall.material.opacity = 0.34 + 0.1 * Math.sin(G.phase * 5);
    }
    ...
    if (guard.visible) {
      ...
      animateChar(guard, {
        run: true,
        phase: G.phase * 0.95,
        lunge: Math.max(0, Math.min(1, (gd - 0.55) / 0.45)), // reaches for you as he closes
